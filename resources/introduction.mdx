---
title: 'Resource Center'
description: 'Programs, tools, and information to help you get the most from RepoCloud'
---

# Resource Center

Welcome to the RepoCloud Resource Center. This section provides access to programs, guidelines, and additional resources to help you maximize the value of the RepoCloud platform.

## Programs and Opportunities

RepoCloud offers various programs to engage with our community and  ecosystem:

<CardGroup cols={2}>
  <Card title="Affiliate Program" icon="handshake" href="/resources/affiliate-program">
    Earn recurring commissions by referring customers to RepoCloud
  </Card>
  <Card title="Submit Your App" icon="cloud-upload" href="/resources/submit-app">
    Add your application to the RepoCloud marketplace
  </Card>
</CardGroup>

## Partner Resources

For RepoCloud partners, we provide comprehensive resources to support your integration and success:

<CardGroup cols={2}>
  <Card title="Partner FAQ" icon="question-circle" href="/resources/partner-faq">
    Frequently asked questions about the RepoCloud Partner Program
  </Card>
  <Card title="Documentation Guidelines" icon="file-alt" href="/resources/app-documentation-guidelines">
    Best practices for creating effective application documentation
  </Card>
  <Card title="API Reference" icon="code" href="/resources/api-reference">
    Technical details for integrating with the RepoCloud platform
  </Card>
  <Card title="Marketing Guide" icon="bullhorn" href="/resources/partner-marketing">
    Tips and resources for marketing your RepoCloud integration
  </Card>
</CardGroup>

## Legal Documents

Access our legal documents and policies:

<CardGroup cols={2}>
  <Card title="Privacy Policy" icon="shield" href="/resources/privacy-policy">
    Our commitment to protecting your privacy and data
  </Card>
  <Card title="Terms & Conditions" icon="gavel" href="/resources/terms-conditions">
    The terms governing use of RepoCloud services
  </Card>
</CardGroup>

## Getting Involved

RepoCloud is more than just a platform—it's a community of developers, businesses, and technology enthusiasts. We encourage you to:

1. **Participate in our programs** - Become an affiliate or submit your application
2. **Provide feedback** - Help us improve by sharing your thoughts and suggestions
3. **Connect with us** - Follow us on social media and join our community discussions
4. **Stay updated** - Subscribe to our newsletter for the latest announcements and updates

## Contact Us

If you have questions about any of our resources or programs, please contact us at [resources@repocloud.io](mailto:resources@repocloud.io). 